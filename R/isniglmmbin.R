...
#' @param nAGQ  integer scalar - the number of points per axis for evaluating the adaptive Gauss-Hermite approximation to the log-likelihood for
...
#'              coefficients in the penalized iteratively reweighted least squares step. (See documentation for glmer() for Details.)
...
#' @name isniglmmbin
#' @import  matrixcalc mvtnorm nnet  stats Formula
...
#' ##summary(result)
#'
isniglmmbin = function(formula, data, random, id, weights, subset, predprobobs, misni=FALSE, nAGQ=10, method=1) {
 ...
 ## (1) process the call and set up model frame.
  ...
  if (length(random)==2) {
    ...
    if (method==1){
      ...
      ##          at present implemented only for models with a single scalar random effect. Use method=1  for vector random effects.")
      ## }

      print("Fitting mixed logit models assuming MAR using glmer() function...")
      glmerres=glmer(formula=fglmer, data =mf, family=binomial,na.action=na.omit,nAGQ=nAGQ)
